...
pub fn ray_sphere() -> ImageBuffer<image::Rgb<u8>, Vec<u8>> {
    ...
    let camera_pos = Vec3A::new(0., 0., 0.);

    // direction to render pixels in
    let viewport_x = Vec3A::new(viewport_width, 0., 0.);
    let viewport_y = Vec3A::new(0., -viewport_height, 0.); // invert viewport height to start top and go to bottom
    ...
    let viewport_top_left = camera_pos - Vec3A::new(0., 0., focal_length) - viewport_x/2. - viewport_y/2.;
...
fn ray_color(ray: &Ray) -> Vec3A {
    if hit_sphere(&Vec3A::new(0., 0., 1.0), 0.5, ray) {
        return Vec3A::new(1., 0., 1.);
    }
...
fn hit_sphere(center: &Vec3A, radius: f32, ray: &Ray) -> bool {
